...
#'
#' @param data a list containing the data (in data.frame) for fitting each the individual curves (Age-Length data if type="VBG" or Weight-Length data if type="WL").
#' @param type the selected type of curve: Von Bertalanfly growth curve (type="VBG) and length-weight curve (type="WL")
#' @param parameters the corresponding parameters according the selected type. If type="VBG", L_inf=Asymptotic average maximum body size, k=Growth rate coefficient that determines how quickly the maximum is attained, and t0=Hypothetical age at which the species has zero length. If type="WL", a=Allometric growth parameter and b=Scaling constant.
